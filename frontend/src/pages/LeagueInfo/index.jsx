...
// import data from "../../services/exemple";
...
function Index() {
  ...
  useEffect(() => {
    const getLeague = async () => {
      ...
      if (!res.data.error) {
        ...
        //   i.rounds.filter((l) => {
        //     if (
        //       moment.unix(l.date_unix).format("DD/MM/YYYY") ==
        //       new Date().toLocaleDateString("pt-BR")
        //     ) {
        //       dateLive.push(l);
        //     }
        //   });
        //   document.title = `Shaftscore: Campeonato - ${data.league_country} - ${data.league_name}`;
        // });
      }
    };
    getLeague();
  ...
  //   if (data) {
  //     for (var i = 1; i <= 38; i++) {
  //       var filterWeek = data.greek.filter((e) => e.game_week === i);
  //       dateFilter.push(filterWeek);
  //     }
  //     setWeek(dateFilter);
  //   }
  // }, [data]);
  table &&
    table.league_table.map((col) => {
      zones.push(col.zone.name);
    });
  zones = [...new Set(zones)];
    ...
    <Main>
      ...
      {data && (
        <>
          ...
          {/* <Title>Estatísticas do clube</Title> */}
          <Heatmap>
            {/*  Gols marcados */}
            {/* <EstatísticasClub>
              <div className="header">
          ...
          <Informations>
            ...
            <span>
              Resultados {data.leagueName} ao vivo, Serie A, placar ao vivo,
              Serie A. Siga resultados.
            </span>
            <span>
              {data.leagueName} e várias ligas competições de futebol no mundo
              inteiro.
            </span>
            <span>
