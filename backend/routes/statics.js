        ...
        if (isGetScpPage) {
            ...
            if (!isRefetching) {
                ...
                setTimeout(async () => {
                    ...
                    await redis.set(`${key}:validation`, true, 'ex', 60);// CACHE Demora 30 segundos. PADRÂo 1 h
                }, 0);
            };
        };
        ...
        if (isGetScpPage) {
            ...
            if (!isRefetching) {
                ...
                setTimeout(async () => {
                    console.log(`[_${key}_] Atualizando CACHE...`);
                    await redis.set(key, JSON.stringify(await scpPage()));
                    await redis.set(`${key}:validation`, true, 'ex', 20);// CACHE Demora 30 segundos. PADRÂo 1 h
                }, 0);
            };
        };
        if (getScpPage) return res.status(201).json({ error: false, data: JSON.parse(getScpPage) });
