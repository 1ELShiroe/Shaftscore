...
router.get('/img/', async (req, res) => {
    ...
    try {
        ...
        const dir = path.join(__dirname, `../assets/${type == 'country' ? 'country' : 'leagues'}/${name.toLowerCase()}.png`)
        if (type == 'country' && dir) return res.sendFile(dir)
        else if (type == 'league' && dir) return res.sendFile(dir)
    } catch (err) {
